...
/* Example code
try{
    $statement = new DBstatement('SELECT * FROM user WHERE username = :username');
...
*/
$usrnm = 'admin';
$pswrd = password_hash('admin', PASSWORD_DEFAULT);
try{
    $stmt = new raymondoor\DBstatement("INSERT INTO user (username, password) VALUES (:username, :password)");
    $result = $stmt->execute([':username' => $usrnm, ':password' => $pswrd]);
    echo $result;
}catch(Exception $e){
    echo $e->getMessage();
}
